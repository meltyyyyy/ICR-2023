...
cust_color = ["#EDC7B7", "#EEE2DC", "#BAB2B5", "#123C69", "#AC3B61"]
...
features_std = (
    train.loc[:, num_cols].apply(lambda x: np.std(x)).sort_values(ascending=False)
)
f_std = train[features_std.iloc[:20].index.tolist()]

with pd.option_context("mode.use_inf_as_na", True):
    features_skew = np.abs(
        train.loc[:, num_cols]
...
with pd.option_context("mode.use_inf_as_na", True):
    features_kurt = np.abs(
        train.loc[:, num_cols]
        .apply(lambda x: np.abs(kurtosis(x)))
        .sort_values(ascending=False)
    ).dropna()
...
def feat_dist(df, cols, rows=3, columns=3, title=None, figsize=(30, 25)):
    ...
    for i, j in zip(cols, axes):
        ...
        j.set_title(
            "Dist of {0} Norm Fit: $\mu=${1:.2g}, $\sigma=${2:.2f}".format(
                i, mu, sigma
            ),
            weight="bold",
        )
...
feat_dist(
    ...
    rows=2,
    columns=4,
    title="Distribution of High Std Features",
    figsize=(30, 8),
)
...
feat_dist(
    train,
    kurt_f.columns.tolist(),
    rows=2,
    columns=4,
    title="Distribution of High Kurtosis Features",
    figsize=(30, 8),
)
...
sns.heatmap(
    sorted_correlations.iloc[1:].to_frame()[sorted_correlations >= 0.15],
    cmap="inferno",
    annot=True,
    vmin=-1,
    vmax=1,
...
correlations = correlations[
    correlations["level_0"] != correlations["level_1"]
]  # preventing 1.0 corr
...
for train_index, val_index in skf.split(train, greeks.iloc[:, 1:-1]):
    X_train, X_val = (
        train.loc[train_index, num_cols + [cat_cols]],
        train.loc[val_index, num_cols + [cat_cols]],
    )
...
def balance_logloss(y_true, y_pred):
    ...
    logloss = (
        -1 / nc[0] * (np.sum(np.where(y_true == 0, 1, 0) * np.log(y_pred[:, 0])))
        - 1 / nc[1] * (np.sum(np.where(y_true != 0, 1, 0) * np.log(y_pred[:, 1])))
    ) / 2
