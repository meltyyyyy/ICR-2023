...
def feature_eng(train, test, n_components=2):
    ...
    combined = pd.concat([train[new_num_cols], test[new_num_cols]], axis=0)
    
    # Fit and transform the combined data with t-SNE
    tsne = TSNE(n_components=2, random_state=42)
    combined_tsne = tsne.fit_transform(combined)
    
    # Split the transformed data back into train and test
    df_tsne = pd.DataFrame(combined_tsne[:len(train)], columns=['tsne_1', 'tsne_2'])
    test_df_tsne = pd.DataFrame(combined_tsne[len(train):], columns=['tsne_1', 'tsne_2'])
    
    df = pd.concat([train[['Id', 'Class']], df_tsne], axis=1)
    test_df = pd.concat([test[['Id']], test_df_tsne], axis=1)
...
def plot_tsne(df, labels):
    ...
    for label in np.unique(labels):
        mask = labels == label
        plt.scatter(df[mask, 0], df[mask, 1], s=30, label=str(label))
...
def main():
    ...
    plot_tsne(df.values, df['Class'].values)
    feat_cols = df.columns.drop(["Class", "Id"]).tolist()
    target = "Class"
